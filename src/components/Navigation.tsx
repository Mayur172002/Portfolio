...
const Navigation = () => {
  ...
  return (
    <>
      ...
      <div 
        ref={mobileMenuRef}
        className="fixed top-0 right-0 w-full h-full bg-background/95 backdrop-blur-lg z-50 transform translate-x-full md:hidden"
      >
        ...
        <div className="flex flex-col space-y-6 p-6 mt-8">
          {navItems.map((item) => (
            <button
              key={item.id}
              onClick={() => scrollToSection(item.id)}
              className="menu-item text-left text-xl text-foreground/80 hover:text-primary-glow transition-colors duration-300"
            >
              {item.name}
            </button>
          ))}
          ...
          <button 
            onClick={() => scrollToSection('contact')}
            className="menu-item px-6 py-3 bg-gradient-primary text-primary-foreground rounded-lg hover:shadow-glow-primary transition-all duration-300 text-center mt-8"
          >
            Hire Me
          </button>
        </div>
